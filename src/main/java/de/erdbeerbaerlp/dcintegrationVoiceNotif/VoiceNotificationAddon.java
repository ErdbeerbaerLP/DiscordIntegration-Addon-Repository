...
public class VoiceNotificationAddon implements DiscordIntegrationAddon, EventListener {
    ...
    public void onEvent(GenericEvent event) {
        if (event instanceof GenericGuildVoiceUpdateEvent) {
            GenericGuildVoiceUpdateEvent evt = (GenericGuildVoiceUpdateEvent) event;
            final VoiceChannel joinedChannel = evt.getChannelJoined();
            final VoiceChannel leftChannel = evt.getChannelLeft();
            if (cfg.voiceChannels.length == 0 || (joinedChannel != null && ArrayUtils.contains(cfg.voiceChannels, joinedChannel.getId())) || (leftChannel != null && ArrayUtils.contains(cfg.voiceChannels, leftChannel.getId()))) {
                if (event instanceof GuildVoiceJoinEvent) {
                    ...
                    GuildVoiceMoveEvent ev = (GuildVoiceMoveEvent) event;
